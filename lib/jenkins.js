...
function Jenkins(runner, options) {
  ...
  options.jenkins_screenshots_path =  process.env.JENKINS_SCREENSHOTS_PATH || options.jenkins_screenshots_path  || '/';
    ...
    tests.forEach(function(test) {
      ...
      if (options.screenshots) {
        ...
        if (options.screenshots == 'loop') {
          ...
          screenshot = options.jenkins_screenshots_path 
              + currentSuite.suite.fullTitle().split(' ').join('_')
              + '_' + test.title.split(' ').join('_')
              + "." + imagetype.replace(/("|'|\(|\)|\/|\\)/img, '');
        }
